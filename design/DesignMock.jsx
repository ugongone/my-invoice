...
import { Printer, Plus, Trash2, RotateCcw, Save } from 'lucide-react';

const SimpleInvoiceApp = () => {
  // 今日の日付を取得
  ...
  // --- 永続化（LocalStorage） ---
  useEffect(() => {
    const savedData = localStorage.getItem('simple_invoice_v1');
    if (savedData) {
      try {
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem('simple_invoice_v1', JSON.stringify(data));
    }
  }, [data, isLoaded]);
  ...
  // ※実務では100万円超えの計算ロジックが必要な場合がありますが、シンプル化のため一律10.21%としています
...
export default SimpleInvoiceApp;